...
const int up    =A2;                        //Controller button pin variables
const int down  =A3;                        //"
const int okay  =A0;                        //"
const int back  =A1;                        //"
int btnStateOkay,btnStateDown,btnStateUp,btnStateBack=0; //Button state variables
...
int cursorPosition=0; //Variable that stores position of user nonQuantified LCD selections

int germDuration=8; //hours premature seed requires for germination
int waterCycles =2; //(times water fill/drain per day)
int nextWater   =0; //Hours until next water, used once during initial setup

int reservoirFillTime =2000;                //Time it takes to fill reservoir
int reservoirDrainTime=2000;                //Time it takes to drain reservoir
...
void setup() 
{
  ...
  pinMode(okay,INPUT);    //Sets button pins as inputs
  pinMode(up,INPUT);      //"
  pinMode(down,INPUT);    //"
  pinMode(back,INPUT);    //"

  pinMode(motor1Speed,OUTPUT);  //Set motor pins as outputs
  ...
  //Display Window prompting user to start cycle from sprout(7) or seed(1)
  if(sproutOrSeedWindow()==1) //Cursor position is 1 so Start from Seed
    {
      loadSeedWindow();    
      state="Ger";stage="FIL";fillReservoir=true; //Set program position determined by result of previous window
    }
  else                       //Cursor position is 7 so Start from Sprout
    {
...
void welcomeWindow()
{
  //Displays welcome window to user
  lcd.setCursor(0,0);             //Set cursor to first line
  lcd.print(" Sprouter  V1.0");   //Print first line to LCD
  lcd.setCursor(0,1);             //Set cursor to second line
  lcd.print("    Welcome!");      //Print second line to LCD
  delay(400);                     //Hold display for .4 Seconds
  lcd.clear();                    //Clear display for new input
}
void germDurationWindow()
{
    //Allows user to set duration of soad for premature seeds
    lcd.home();                         //Go to the start of the first line
    lcd.print(" Germ Duration:");       //Print first line
    lcd.setCursor(0,1);                 //Set cursor to second line
    lcd.print("     ");                 //print partial second line
    ...
    delay(400);                         //Hold screen for atleast .4 seconds before grabbing button values    
